...
write_out_location = 'EPA_Trial_Run/run_results_continuous'#'EPA_Trial_Run/run_results_loop_test_corrected_distributions'
Set_periodic_threshold = 5 #detection threshold of the periodic survey in kg/hr 

#LOOPING INSTRUCTIONS
cadence_values = {'monthly': 30, 'bimonthly': 60, 'quarterly': 90, 'semiannual':180, 'annual': 365}
...
cont_rep_delay = 10 #continuous monitoring survey repair delay in days 
...
points = [[0.347], [0.521], [0.694], [1.042], [1.389]] 
time_to_detect_days = [np.inf, 5, 1, 0.5, 0]

#Alt Construction: Continuous monitor as a component monitoring system: 
survey_speed_comp_cont = 1000 #Survey speed in components/hour 
suvey_labor_cost_cont = 100 #survey cost USD/HR 
survey_ophrs_cont = {'begin': 0, 'end': 24}
cont_detection_points= np.array([0.347, 0.521, 0.694, 1.042, 1.389]) #Detection distribution, g/s
cont_detection_probabilities= np.array([0, 0.25, 0.5, 0.75, 1]) #Detecion Distribution, probabilities 
...
def define_detection_methods(timeobj):
    ...
    RD_cont = Dm.repair.Repair(repair_delay = copy.deepcopy(cont_rep_delay))
    ...
    ogi_called_survey_cont = Dm.comp_survey.CompSurvey(
        timeobj,
        survey_interval=None,
        survey_speed=copy.deepcopy(survey_speed_comp_ogi),
        ophrs=copy.deepcopy(survey_ophrs_ogi),
        ...
        detection_probability_points=copy.deepcopy(ogi_detection_points),
        detection_probabilities= copy.deepcopy(ogi_detection_probabilities),
        dispatch_object= copy.deepcopy(RD_cont),
        site_queue=[],
    )
    ...
    cont_monitor_component = Dm.comp_survey.CompSurvey(
        timeobj,
        survey_interval=1,
        survey_speed= copy.deepcopy(survey_speed_comp_cont),
        ophrs= copy.deepcopy(survey_ophrs_cont),
        labor= copy.deepcopy(suvey_labor_cost_cont),
        detection_variables={'flux': 'mean'},
        detection_probability_points= copy.deepcopy(cont_detection_points),
        detection_probabilities= copy.deepcopy(cont_detection_probabilities),
        dispatch_object= copy.deepcopy(RD_cont),
        site_queue=[]
    )
	
    #### 
   #Setting up the continuous monitors: 
    # 
    cont_monitor = Dm.site_monitor.SiteMonitor(
        timeobj,
        time_to_detect_points=copy.deepcopy(points),
        time_to_detect_days=copy.deepcopy(time_to_detect_days),
        detection_variables={'flux': 'mean'},
        site_queue=list(range(gas_field.n_sites)),
        dispatch_object=copy.deepcopy(ogi_called_survey_cont),
    ...
    return ogi_quarterly, ogi_called_survey, ogi_called_survey_sat, ogi_annual_survey, periodic_survey, large_event_survey, RD_ogi, RD_cont, ogi_called_survey_cont, cont_monitor, cont_monitor_component


def define_ldar_programs(gas_field, ogi_quarterly, ogi_annual_survey, periodic_survey, large_event_survey, RD_ogi, RD_cont, ogi_called_survey_cont, cont_monitor, cont_monitor_component): ### RS Edits (5/18/22)
    """
    ...
    # periodic survey with a satellite and an annual ogi: 
    ...
    # periodic survey with a satellite and annual OGI, the satellite portion:
    ...
    # The continuous monitor, set up as continuous: 
    cont_survey = copy.deepcopy(cont_monitor)
    cont_survey.dispatch_object = copy.deepcopy(ogi_called_survey_cont)
    #continuous monitoring, set up as a component survey:
    cont_survey_comp = copy.deepcopy(cont_monitor_component)
    cont_survey_comp.dispatch_object = copy.deepcopy(RD_cont)
    ...
    #Continuous Survey, with called OGI followup
    tech_dict = {
        'continuous': cont_survey,
        'called_ogi': cont_survey.dispatch_object
    }
    cont_survey_LDAR = Dm.ldar_program.LDARProgram(
        copy.deepcopy(gas_field), tech_dict,
    )
  
    #Continuous Survey, with called OGI followup AND an annual OGI survey. 
    tech_dict = {
        'continuous': cont_survey,
        'called_ogi': cont_survey.dispatch_object,
        'annual_ogi': copy.deepcopy(ogi_annual_survey)
    }
    cont_survey_LDAR_annual = Dm.ldar_program.LDARProgram(
        copy.deepcopy(gas_field), tech_dict,
    )
  
    #continuous survey, constructed as a component survey, with an OGI call.   
    # tech_dict = {
    #     'continuous_components': cont_survey_comp, 
    #     'called_ogi': cont_survey_comp.dispatch_object
    # }
    cont_survey_comp_LDAR = Dm.ldar_program.LDARProgram(
        copy.deepcopy(gas_field), {'ogi': cont_survey_comp},
    )
    #Note: If you call a non-ogi dispatch object, you need the tech_dict for the LDARprogram to not define the dispatch object seperately, the way that do with the called OGI surveys. 
    #okay - the component surveys can't call ogi; the continuous LDAR techs can ONLY call ogi. that's really interesting. 
    ...
    ldar_dict = {
        ...
        'continuous' : cont_survey_LDAR,
        'continuous_annual' : cont_survey_LDAR_annual,
        'continuous_comp' : cont_survey_comp_LDAR
    }
    ###
...
for iter in cadence_values:
    ...
    for ind in range(n_montecarlo):
        ...
        ogi_quarterly, ogi_called_survey, ogi_called_survey_sat, ogi_annual_survey, periodic_survey, large_event_survey, RD_ogi, RD_cont, ogi_called_survey_cont, cont_monitor, cont_monitor_component = define_detection_methods(timeobj) 
        ldar_dict = define_ldar_programs(gas_field, ogi_quarterly, ogi_annual_survey, periodic_survey, large_event_survey, RD_ogi, RD_cont, ogi_called_survey_cont, cont_monitor, cont_monitor_component)
